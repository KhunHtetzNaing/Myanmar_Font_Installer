...
Sub Globals
	...
	Dim mm As Typeface : mm = mm.LoadFromAssets("Myanmar.ttf")
	Dim td As Label
...
Sub Activity_Create(FirstTime As Boolean)
	...
	td.Initialize("")
	td.Text = "*ထည့္သြင္းမည္*  ကိုႏွိပ္ပါ။ ၿပီးရင္ေအာက္ပါ *ေဖာင့္ေျပာင္းမည္(၁)* ကိုႏွိပ္ပါ Myanmar.txj (သို႔) Myanmar.itz ကိုဖြင့္ၿပီး Apply လုပ္ေပးလိုက္ပါ မရခဲ့ရင္ *ေဖာင့္ေျပာင္းမည္(၂)* ကိုႏွိပ္ၿပီး Myanmar Font နာမည္နဲ႔ Theme ကိုေရြးေပးလိုက္ပါ။ နဂိုမူလေဖာင့္ကိုျပန္ထားခ်င္ရင္ *ေဖာင့္ေျပာင္းမည္(၂)* ကိုႏွိပ္ၿပီး Default ကိုျပန္ေရြးထားနိုင္ပါတယ္။"
	...
	B.Initialize("B","ca-app-pub-4173348573252986/6444280558")
	Dim height As Int
	...
	I.Initialize("I","ca-app-pub-4173348573252986/7921013758")
	I.LoadAd
End Sub
...
Sub b1_Click
	...
	If File.Exists(File.DirRootExternal & "/Vivo Myanmar Font","Myanmar.txj") = True Then File.Delete(File.DirRootExternal,"Vivo Myanmar Font/Myanmar.txj")
	File.Copy(File.DirAssets,"Myanmar.txj",File.DirRootExternal & "/Vivo Myanmar Font","Myanmar.txj")
	
	If File.Exists(File.DirRootExternal & "/Vivo Myanmar Font","Myanmar.itz") = True Then File.Delete(File.DirRootExternal,"Vivo Myanmar Font/Myanmar.itz")
	File.Copy(File.DirAssets,"Myanmar.itz",File.DirRootExternal & "/Vivo Myanmar Font","Myanmar.itz")
...
Sub b3_Click
	...
	If File.Exists(File.DirRootExternal & "/Download/i Theme/Font","Myanmar.txj") = True Then File.Delete(File.DirRootExternal,"Download/i Theme/Font/Myanmar.txj")
	File.Copy(File.DirAssets,"Myanmar.txj",File.DirRootExternal & "/Download/i Theme/Font","Myanmar.txj")
	
	If File.Exists(File.DirRootExternal & "/Download/i Theme/Font","Myanmar.itz") = True Then File.Delete(File.DirRootExternal,"Download/i Theme/Font/Myanmar.itz")
	File.Copy(File.DirAssets,"Myanmar.itz",File.DirRootExternal & "/Download/i Theme/Font","Myanmar.itz")
	Try
		Dim ii As Intent
		ii.Initialize("", "")
...
Sub share_Click
	...
	copy.clrText
	copy.setText("#MyanmarFontInstaller App! Myanmar Font Installer App For Android! You can Use in Samung, Oppo,Vivo, Huawei (EMUI) and Xiaomi (MIUI) without Root Access!!!! Download Free at : http://www.MyanmarAndroidApp.com/search?q=Myanmar+Font+Installer")
